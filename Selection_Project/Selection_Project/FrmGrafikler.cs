...
namespace Selection_Project
{
    public partial class FrmGrafikler : Form
    {
        ...
        {
            //İlçe Adlarını Combobox'a çekmek için

            baglanti.Open();
            SqlCommand komut = new SqlCommand("SELECT ILCEAD FROM TBLILCE", baglanti);
            SqlDataReader dr = komut.ExecuteReader();      //SqlDataReader ile verileri okuruz. komut. ile de db'den çetiğimiz verileri ilişkilendiririz.

            while (dr.Read())   // dr komutu okuma yaptığı müddetçe
            {
                comboBox1.Items.Add(dr[0]);   // ILCEAD içindeki 0'ıncı indexi combobox'a ekle
            }
            baglanti.Close();
